...
const Sidebar = () => {
   ...
   try {
    ...
    const{data}=await axios.post("/api/owner/updated-image",formData);
    if(data.success){
        fetchUser();
  ...
  return (
    <div className='relative min-h-screen md:flex md:flex-col items-center pt-8 max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
    ...
    {ownerMenuLinks.map((link, index) => (
        <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path===location.pathname?"bg-primary/10 text-primary":"text-gray-600"}`}>
            ...
            <div
                className={`${
                    link.path === location.pathname &&
                    'bg-primary w-1 h-8 rounded-l right-0 absolute'
                }`}
            ></div>
        </NavLink>
    ))}
