...
const Navbar = () => {
  ...
  return (
    ...
    className={`flex items-centre justify-between px-6 md:px-16 lg:px-24 
    xl:px-32 py-4 text-gray-600 border-b border-borderColor relative transition-all ${location.pathname==="/" && "bg-light"}`}>
     ...
     items-start sm:items-centre gap-4 sm:gap-8 max-sm:p-4 transition-all duration-300 z-50  ${location.pathname === "/" ? "bg-light" : "bg-white"} ${open ? "max-sm:translate-x-0":"max-sm:translate-x-full"}`}>
        ...
        <div className='hidden lg:flex items-centre text-sm gap-2 border border-borderColor px-3 rounded-full max-w-56'>
